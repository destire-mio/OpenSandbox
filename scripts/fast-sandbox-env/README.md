...
and the source-built OpenSandbox **server** (fleets runtime) and **ingress
...
```
SDK ──HTTP──> server (source-built, runtime.type=fleets)
...
SDK ──header──> ingress gateway (source-built, --provider-type=fleets)
...
- **execd** is baked into the SandboxTemplate golden image. The template
  builder pulls the image referenced by `EXECD` anonymously from its
  registry, so it is the published `opensandbox/execd` image by default;
  `SBX_IMAGE` / `EXECD` select the build inputs.
...
manifests/
  ...
  opensandbox/server.yaml      lifecycle server: fleets runtime config,
                               NodePort 30880 -> host 8080
  opensandbox/ingress-gateway.yaml   fleets provider + FastPath + shared
...
asserted) → SandboxTemplate golden image → SandboxPool (fastlet Ready,
egress Ready, pool conditions, actions protocol check) → OpenSandbox
server + ingress gateway → end-to-end verify (server create → signed
gateway route → execd `/ping` → delete).
